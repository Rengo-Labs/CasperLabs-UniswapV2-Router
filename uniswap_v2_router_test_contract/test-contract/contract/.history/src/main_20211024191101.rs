...
fn add_liquidity_cspr() {
    ...
    let self_package_hash: Key = runtime::get_named_arg("self_package_hash");
    let self_package_hash: ContractHash = ContractHash::from(self_package_hash.into_hash().unwrap_or_revert());
    ...
    // this entry points context is session therefore it can't access contract keys. Therefore to set the key, it calls new entrypoint method.
    let _: () = runtime::call_contract(self_package_hash, "set_liquidity_cspr_keys", runtime_args! { "amount_token" => amount_token, "amount_cspr" => amount_cspr, "liquidity" => liquidity});
}
...
fn get_entry_points() -> EntryPoints {
    ...
    entry_points.add_entry_point(EntryPoint::new(
        ...
        vec![
            ...
            Parameter::new("self_package_hash", CLType::Key),
        ],
        <()>::cl_type(),
        EntryPointAccess::Public,
