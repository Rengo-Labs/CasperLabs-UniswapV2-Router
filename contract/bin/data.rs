...
pub const CALLER_HASH: &str = "caller_hash";
...
pub fn hash() -> ContractPackageHash
{
    get_key(PACKAGE_HASH).unwrap_or_revert().unwrap_or_revert()
}

pub fn set_hash(package_hash: Key)
{
    set_key(PACKAGE_HASH, package_hash);
}

pub fn set_caller(caller_hash: Key) {
    set_key(CALLER_HASH, caller_hash);
}
