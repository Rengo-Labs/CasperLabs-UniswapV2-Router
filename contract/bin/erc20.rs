...
pub trait ERC20<Storage: ContractStorage>: ContractContext<Storage> {
    fn init(&mut self, name: String, symbol: String, decimals: u8, package_hash: Key) {
        data::set_name(name);
        data::set_symbol(symbol);
        data::set_decimals(decimals);
        data::set_hash(package_hash);
        data::set_caller(self.get_caller());
        Balances::init();
        Allowances::init();
    }

    fn get_hash() -> ContractPackageHash {
        data::hash()
    }
